/**
 * FAQ-OpenAI.gs - Google Apps Script for serving FAQ content
...
function doGet(e) {
  ...
  // Simple Q&A endpoint using OpenAI (stateless - no memory)
  if (action === 'ask' && query) {
    try {
      const response = generateSimpleResponse(query);
...
/**
 * Simple response using OpenAI (no conversation memory)
 * For chatbot with memory, use ChatBot.gs instead
 */
function generateSimpleResponse(query) {
  ...
  const prompt = `You are a helpful assistant for Barbie's Bail Bonds in Palm Beach County, Florida.
Use the following FAQ information to answer the user's question. Be concise and helpful.
...
Response:`;
  ...
  const response = UrlFetchApp.fetch('https://api.openai.com/v1/chat/completions', {
    ...
    payload: JSON.stringify({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a helpful assistant for Barbie\'s Bail Bonds. Answer questions about bail bonds, the process, and services offered.'
        },
        {
          role: 'user',
          content: prompt
  ...
  return {
    ...
    sources: relevantItems.slice(0, 2).map(item => ({
      question: item.question,
      similarity: Math.round(item.similarity * 100) / 100
    }))
  };
...
<body>
  ...
  <p>Find answers to common questions about bail bonds and our services. We have ${faqData.length} FAQs available.</p>
  ${faqData.map((item, index) => `
    <div style="margin-bottom: 20px; padding: 15px; border: 1px solid #ddd; border-radius: 5px;">
      <h3 style="color: #F28C00; margin-bottom: 10px;">${item.q}</h3>
      <p style="color: #333; line-height: 1.6;">${item.a}</p>
    </div>
  `).join('')}
</body>
</html>
...
// Import semanticSearch from RAG-OpenAI.gs when deployed together
// Or deploy RAG-OpenAI.gs separately and call via UrlFetchApp