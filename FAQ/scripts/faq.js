  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    function searchFAQ() {
      ...
      faqData.forEach((faqItem, index) => {
        ...
        const questionMatch = faqItem.q.toLowerCase().includes(searchTerm);
        const answerMatch = faqItem.a.toLowerCase().includes(searchTerm);
        ...
        if (!searchTerm || questionMatch || answerMatch) {
          ...
          const answerInner = element.querySelector('.answer-inner p');
          ...
          if (searchTerm) {
            ...
            questionSpan.innerHTML = highlightText(faqItem.q, searchTerm);
            btn.appendChild(questionSpan);
            
            // Update answer text with highlighting
            if (answerInner) {
              answerInner.innerHTML = highlightText(faqItem.a, searchTerm);
            }
          } else {
            ...
            btn.innerHTML = '';
            btn.appendChild(chevron);
            btn.appendChild(document.createTextNode(' ' + faqItem.q));
            
            if (answerInner) {
              answerInner.textContent = faqItem.a;
            }
          }
        } else {
    ...
    if (searchInput) {
      ...
      // Initial search info
      searchInfo.textContent = `Showing all ${faqData.length} questions`;
    }
  });
})();
